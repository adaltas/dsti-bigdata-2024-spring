...
Start a new project directory and add two sub-directories, `./source` and `./sink`. Make sure to be in the base level of the project directory when you start the lab. Add a "hello-nifi.txt" file to your base directory and copy it to the source directory. 
...
1. Run NiFi Docker image with the following command:
...
2. Run The NiFi-Registry Docker image with the following command:
...
3. In a terminal run `docker inspect nifi-registry-latest` and make note of the "Gateway" ip.
4. Access the NiFi-Registry ui here: http://localhost:18080/nifi-registry
...
9. Edit the client to add the registry url (http://dockercontainer.gateway.ip.here:18080).
