...
2. Copy the `nyc_drivers` folder to your user directory in HDFS:
   ...
   ```bash
   hdfs dfs -mkdir -p "/education/$GROUP/$USER/lab4"

   hdfs dfs -cp /education/$GROUP/resources/lab4/nyc_drivers "/education/$GROUP/$USER/lab4"
   ```
...
4. Create an external table targeting our data with this statement (to be completed, replace `YOUR_USERNAME`):
   ...
   ```sql
   SET hivevar:clusterUsername='YOUR_USERNAME';
   -- DO NOT USE '.' NOR '-' IN HIVEUSERNAME
   SET hivevar:hiveUsername='YOUR_USERNAME';

   CREATE EXTERNAL TABLE dsti_2024_s24_1.${hiveUsername}_nyc_drivers_ext (
     driver_id INT,
     -- COMPLETE HERE
   )
...
2. Check that your table was created using the HDFS CLI at `/warehouse/tablespace/managed/hive/${GROUP}.db/${USER}_nyc_drivers` (should be empty)
...
3. Check what the data looks like in the managed table using the HDFS CLI at `/warehouse/tablespace/managed/hive/${GROUP}.db/${username}_nyc_drivers`
